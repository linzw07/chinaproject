...
jQuery(document).ready(function() {
		...
		jQuery(window).load(function() {jQuery('.newspaper').show().masonry({ itemSelector : '.newspaper_column', isAnimated: true});});

			jQuery('.portfolio_container').masonry({ 
		...
		//run masonry when window is resized
			...
			jQuery(window).resize(function(){
			  clearTimeout(timer);
			  setTimeout( MasonrySetSize, 10);
			})
	
	});
		

function MasonrySetSize(){
			   if (jQuery(window).width() < 479) {

			   }
			   else if (jQuery(window).width() < 400) {
jQuery('.newspaper').show().masonry({ itemSelector : '.newspaper_column'});
			   }
