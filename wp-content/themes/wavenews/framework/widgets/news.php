...
class Wave_Popular_News_Widget extends WP_Widget {

	function Wave_Popular_News_Widget() {
		$widget_ops = array( "classname" => "widget_popular_news", "description" => "Displays the Popular news" );
		$this-> WP_Widget( "popular_news", THEME_SLUG . " - Popular News", $widget_ops );
	...
	function widget( $args, $instance ) {
		...
		$cache = wp_cache_get( "Wavesnow_Widget_Popular_News", "widget" );
		...
		if ( !empty( $instance["cats"] ) ) {
			$catSlugs = implode( ",", $instance["cats"] );
		}
		else{
			$catSlugs=array();
			$categories = get_categories( 'orderby=name&hide_empty=0&taxonomy=news_category' );
			foreach ( $categories as $category )
			{
				array_push($catSlugs, $category->term_id);
			}
			$catSlugs = implode( ",", $catSlugs);
				 
		}

		$query = array( 'post_type' => 'news',
	   ...
	   <?php endif; ?>
		...
		wp_cache_set( 'Wavesnow_Widget_Popular_News', $cache, 'widget' );
	...
	function update( $new_instance, $old_instance ) {
		...
		if ( isset( $alloptions['Wavesnow_Widget_Popular_News'] ) )
			delete_option( 'Wavesnow_Widget_Popular_News' );
	...
	function flush_widget_cache() {
		wp_cache_delete( 'Wavesnow_Widget_Popular_News', 'widget' );
	}
...
class Wave_Recent_News_Widget extends WP_Widget {

	function Wave_Recent_News_Widget() {
		$widget_ops = array( "classname" => "widget_recent_news", "description" => "Displays the Recent News" );
		$this-> WP_Widget( "recent_news", THEME_SLUG . " - Recent News", $widget_ops );
	...
	function widget( $args, $instance ) {
		...
		$cache = wp_cache_get( "Wavesnow_Widget_Recent_News", "widget" );
...
class Wave_Related_News_Widget extends WP_Widget {

	function Wave_Related_News_Widget() {
		$widget_ops = array( "classname" => "widget_related_news", "description" => "Displays the Recent News" );
		$this-> WP_Widget( "related_news", THEME_SLUG . " - Related News", $widget_ops );
		$this-> alt_option_name = "widget_related_news";
	...
	function widget( $args, $instance ) {
		$cache = wp_cache_get( "Wavesnow_Widget_Related_News", "widget" );
		if ( !is_array( $cache ) )
			$cache = array();
		if ( isset( $cache[$args['widget_id']] ) ) {
			echo $cache[$args['widget_id']];
	   ...
	   <?php endif; ?>
		...
		$cache[$args['widget_id']] = ob_get_flush();
		wp_cache_set( 'Wavesnow_Widget_Related_News', $cache, 'widget' );
	...
	function update( $new_instance, $old_instance ) {
		...
		$alloptions = wp_cache_get( 'alloptions', 'options' );
		if ( isset( $alloptions['Wavesnow_Widget_Related_News'] ) )
			delete_option( 'Wavesnow_Widget_Related_News' );
	...
	function flush_widget_cache() {
		wp_cache_delete( 'Wavesnow_Widget_Related_News', 'widget' );
	}
