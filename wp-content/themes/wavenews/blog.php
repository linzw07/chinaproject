...
if ( isset( $_GET["blog_layout"] ) ) {
	...
	if ( get_post_meta( $post->ID, '_layout', true ) !== 'default' ) {
		$blog_layout = get_post_meta( $post->ID, '_layout', true );
	} else {

		$blog_layout =  theme_option( THEME_OPTIONS, 'blog_layout' );
	}
...
if ( isset( $_GET["blog_style"] ) ) {
	$loop_style = $_GET["blog_style"];
} else {
	$loop_style = theme_option( THEME_OPTIONS, 'blog_loop_style' );
}
...
<?php
theme_function( 'slideshow', get_queried_object_id() );
theme_function( 'page_gmap', get_queried_object_id() );
theme_function( 'slogan_heading', get_queried_object_id() );
...
if ( $loop_style == 'classic' || $loop_style == 'classic_thumb' ) {
	get_template_part( 'blog_loop_classic', 'blog' );
}
